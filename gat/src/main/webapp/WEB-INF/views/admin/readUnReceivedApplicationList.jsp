		...
		<div class="col-10 offset-lg-1 my-4">
			<table class="table table-hover">
				...
				<%-- <c:set value="${apListVO.appPostVO }" var="appPostVO"> --%>
					<c:forEach items="${apListVO.appPostVO }" var="appPostVO">
						<tr>
							<td>${appPostVO.appNo }</td>
							<td><a href="${pageContext.request.contextPath }/readUnReceivedApplicationDetail.do">${appPostVO.appTitle }</a></td>
							<td>${appPostVO.memberVO.name }</td>
							<td>${appPostVO.appRegdate }</td>
						</tr>
					</c:forEach>
				<%-- </c:set> --%>
				</tbody>

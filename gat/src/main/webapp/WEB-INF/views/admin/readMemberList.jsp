...
<script type="text/javascript">
	function deleteMember(){
		var flag=confirm(id+"님을 삭제하시겠습니까?");
		if(flag){
			location.href="";
		}else{
			return false;
					...
					<c:forEach items="${mListVO.mlist }" var="mlist">
						<c:if test="${mlist.memberGroupVO.mgroupNo!='4' }">
							<tr>
								...
								<td>
									<!-- <input type="button" value="강제탈퇴" onclick="return deleteMember()"
									 class="btn btn-sub-2-gnt btn-block"> -->
									 <form action="#">
									 	<input type="submit" onsubmit="return deleteMember()" value="강제탈퇴" class="btn btn-sub-2-gnt btn-block">
									 </form>
								</td>
							</tr>
						</c:if>
